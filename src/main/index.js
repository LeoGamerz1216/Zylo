...
function runApp() {
  ...
  const ALLOWED_RENDERER_FILES = process.env.NODE_ENV === 'production'
    // __ZYLO_ALLOWED_PATHS__ is replaced by the injectAllowedPaths.mjs script
    ? new Set(__ZYLO_ALLOWED_PATHS__)
    : new Set()
  ...
  app.removeAsDefaultProtocolClient('zylo')

  // If we are running a non-packaged version of the app && on windows
  if (process.env.NODE_ENV === 'development' && process.platform === 'win32') {
    // Set the path of electron.exe and your app.
    // These two additional parameters are only available on windows.
    app.setAsDefaultProtocolClient('zylo', process.execPath, [path.resolve(process.argv[1])])
  } else {
    app.setAsDefaultProtocolClient('zylo')
  }
    ...
    app.on('second-instance', async (_, commandLine, __) => {
      ...
      if (typeof commandLine !== 'undefined') {
        ...
        const readyHandler = (event) => {
          if (isZyloUrl(event.senderFrame.url)) {
  ...
  app.on('ready', async (_, __) => {
    ...
    // Zylo only needs a few permissions, so we reject requests for other permissions
    // and reject all requests on non-Zylo URLs.
    //
    // Zylo needs the following permissions:
    ...
    session.defaultSession.setPermissionRequestHandler((webContents, permission, callback, details) => {
      if (!isZyloUrl(webContents.getURL())) {
  ...
  async function createWindow(
    ...
    newWindow.webContents.setWindowOpenHandler((details) => {
      ...
      // Only handle valid URLs that came from a Zylo page
      if (url !== null && isZyloUrl(newWindow.webContents.getURL())) {
        if (isZyloUrl(url)) {
          createWindow({
            replaceMainWindow: false,
            showWindowNow: true,
  ...
  ipcMain.on(IpcChannels.SET_WINDOW_TITLE, (event, title) => {
    if (isZyloUrl(event.senderFrame.url) && typeof title === 'string') {
  ...
  function relaunch() {
    if (process.env.NODE_ENV === 'development') {
      app.exit(parseInt(process.env.ZYLO_RELAUNCH_EXIT_CODE))
      return
    }
  ...
  nativeTheme.on('updated', () => {
    ...
    allWindows.forEach((window) => {
      if (isZyloUrl(window.webContents.getURL())) {
  ...
  ipcMain.handle(IpcChannels.WRITE_TO_DEFAULT_FOLDER, async (event, filename, arrayBuffer) => {
    if (
      !isZyloUrl(event.senderFrame.url) ||
  ...
  function syncOtherWindows(channel, event, payload) {
    const otherWindows = BrowserWindow.getAllWindows().filter((window) => {
      return window.webContents.id !== event.sender.id && isZyloUrl(window.webContents.getURL())
    })
  ...
  /*
   * Callback when processing a zylo:// link (macOS)
   */
   ...
   * Remove zylo:// protocol if present
   */
  ...
  function baseUrl(arg) {
    let newArg = arg.replace('zylo://', '')
    // add support for authority free url
      .replace('zylo:', '')

    // fix for Qt URL, like `zylo://https//www.youtube.com/watch?v=...`
  ...
  function navigateTo(path, browserWindow) {
    if (browserWindow == null || !isZyloUrl(browserWindow.webContents.getURL())) {
      return
    }
